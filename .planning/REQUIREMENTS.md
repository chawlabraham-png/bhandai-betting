...
- [ ] **CODE-01**: Shared utilities extracted to `lib/utils.js` (sanitize, fmt, timeAgo, showToast, modal helpers)
...
- [ ] **CODE-05**: Inline CSS extracted to shared (`shared.css`) and role-specific CSS files
- [ ] **CODE-06**: Namespace convention applied -- `window.BX` for shared, `window.Admin`/`window.Agent`/`window.Client` for role-specific
...
| CODE-05 | Phase 8: Code Modularization | Pending |
| CODE-06 | Phase 8: Code Modularization | Pending |
