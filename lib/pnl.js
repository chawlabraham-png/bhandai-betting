...
(function (BX) {
   ...
   */
  // Calculate per-outcome position for an event.
  // statusFilter: 'OPEN' (default, for event book bars) or 'ALL' (for portfolio position view)
  BX.calcEventBook = function calcEventBook(eventId, myOrders, allOutcomes, allEvents, statusFilter) {
    var filter = statusFilter || 'OPEN';
    var eventOrds = myOrders.filter(function (o) {
      return o.event_id === eventId && (filter === 'ALL' || o.status === filter);
    });
    ...
    // Get team names from outcomes
    var ocs = allOutcomes.filter(function (o) { return o.event_id === eventId; });
    var favTeam = ev ? (ev.rate_team || (ocs[0] ? ocs[0].title : 'Team 1')) : 'Team 1';
    var otherTeam = ocs[1] ? ocs[1].title : 'Team 2';
    return { favWins: favWins, favLoses: favLoses, count: eventOrds.length, favTeam: favTeam, otherTeam: otherTeam };
  };
